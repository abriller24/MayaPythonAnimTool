...
from PySide2.QtWidgets import QCheckBox, QFileDialog, QLineEdit, QWidget, QPushButton, QListWidget, QAbstractItemView, QLabel, QHBoxLayout, QVBoxLayout, QMessageBox, QSizePolicy
from PySide2.QtGui import QIntValidator, QRegExpValidator
from PySide2.QtCore import QFile, QRegExp, Signal
...
class MayaToUE:
    ...
    def SaveFiles(self):
        ...
        allObjsToExport = allJnts + list(allMeshes)
        mc.select(allObjsToExport, r=True)
    ...
    def SendToUnreal(self):
        utilityPath = os.path.join(mayaPython.srcDir, "unrealUtility.py")
        utilityPath = os.path.normpath(utilityPath)

        meshPath = self.GetSkeletalMeshSavePath().replace("\\", "/")
        animDir = self.GetAnimFolderPath()
        ...
        print(commands)
        remoteExe = remote_execution.RemoteExecution()
        remoteExe.start()
        remoteExe.open_command_connection(remoteExe.remote_nodes)
        remoteExe.run_command(command)
        remoteExe.stop()
...
class MayaToUEWidget(QWidget):
    def __init__(self):
        ...
        self.pickDirectoryBtn = QPushButton("...")
        self.pickDirectoryBtn.clicked.connect(self.PickFileDirectory)
        self.saveFileLayout.addWidget(self.pickDirectoryBtn)

        self.savePreviewLabel = QLabel("")
        self.masterLayout.addWidget(self.savePreviewLabel)

        saveFileBtn = QPushButton("Save")
        saveFileBtn.clicked.connect(self.mayaToUE.SaveFiles)
        self.masterLayout.addWidget(saveFileBtn)


    def PickFileDirectory(self):
    ...
    def AddNewAnimEntryBtnClicked(self):
        ...
        newAnimEntryWidget.entrySubfixChanged.connect(self.UpdateSavePreviewLabel)
